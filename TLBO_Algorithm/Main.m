...
% 显示通过TLBO算法获得的最佳解
display(['通过TLBO算法获得的' [num2str(Fun_name)], '的最佳解是 : ', num2str(Best_pos)]);
% 显示通过TLBO算法找到的目标函数的最佳最优值
display(['通过TLBO算法找到的' [num2str(Fun_name)], '的目标函数最佳值是 : ', num2str(Best_score)]);

% 创建图形窗口并绘制TLBO算法的收敛曲线
figure = gcf; % 获取当前图形窗口
semilogy(TLBO_curve, 'Color', '#b28d90', 'LineWidth', 2); % 绘制半对数曲线
xlabel('迭代'); % x轴标签
